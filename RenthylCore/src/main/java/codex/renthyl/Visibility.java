...
public enum Visibility {
    ...
    private Visibility(boolean inside, boolean partial, Camera.FrustumIntersect intersect) {
        this.inside = inside;
    ...
    public static Visibility get(boolean inside, boolean partial) {
        for (Visibility v : Visibility.values()) {
            if (v.inside == inside && v.partial == partial) {
                return v;
            }
        }
        throw new IllegalArgumentException("No matching enum found for [inside=" + inside + ", partial=" + partial + "]");
    }
    public static Visibility fromFrustumIntersect(Camera.FrustumIntersect intersect) {
        switch (intersect) {
            ...
            default: throw new UnsupportedOperationException(
                    "Unrecognized " + Camera.FrustumIntersect.class.getName() + " enum constant: " + intersect);
        }
    }
